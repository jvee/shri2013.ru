...
var out='<div class="b-person__title b-block__title"><h2 class="b-block__header">'+(it.name)+'</h2>';if(it.profiles){out+='<ul class="b-person__link-list b-block__link-list">';var arr1=it.profiles;if(arr1){var profile,index=-1,l1=arr1.length-1;while(index<l1){profile=arr1[index+=1];out+='<li class="b-person__link-item b-person__link-item_type_'+(profile.name.toLowerCase())+' b-block__link-item"><a href="'+(profile.link)+'">'+(profile.name)+'</a></li>';} } out+='</ul>';}out+='</div><div class="b-block__content">';if(it.photo){out+='<img class="b-person__photo" src="'+(it.photo)+'" alt="'+(it.name)+'">';}out+='<div class="b-person__about b-text">'+( it.about)+'</div></div>';return out;
};
return tmpl;})()